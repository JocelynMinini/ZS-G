...
for itr = 1:length(current_model.Internal.PCE(current_output).DegreeArray)
    ...
    if (itr == 1) || (scores(itr) > bestScore) || best_loo > min(current_model.Internal.PCE.OLS.LOO) % if the current score is the best one, update the indices and coefficients thus far
